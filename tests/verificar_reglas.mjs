...
async function cargarCatalogo () {
  ...
  const csv = readFileSync(join(AQUI, '../data/catalogo_experiencias.csv'), 'utf8').trim().split('\n')
  const cab = csv[0].split(',')
  const filas = csv.slice(1).map(l => {
    const c = l.split(','); return Object.fromEntries(cab.map((k, i) => [k, c[i]]))
  })
...
export function reglasDuras (exp, p) {
  ...
  if (Number(exp.noches) > p.dias) relajables.push(`duración: ${exp.noches} noches con ${p.dias} días`)
  if (!enTemporada(exp.temporada_agencia, p.mes)) relajables.push(`fuera de temporada (${exp.temporada_agencia})`)
  if (veto.includes('julio y agosto') && [7, 8].includes(p.mes)) relajables.push('desaconsejado en julio y agosto')
...
for (const [pid, eid, desc, nivel] of NEGATIVOS) {
  ...
  if (nivel === 'inviolable') {
    ...
    detalle = ok ? 'admitido, como debe ser' : `descartado indebidamente: ${[...relajables, ...inviolables].join(' · ')}`
  }
  if (!ok) fallos++
